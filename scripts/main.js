    ...
    window.RuntimeInterface = class f {
        ...
        _OnEventFromRuntime(a) {
            ...
            try {
                ...
                return console.error(`Exception in '${b}' handler '${c}':`, a), void(null !== e && this._PostResultToRuntime(e, !1, a.toString()))
            }
            null !== e && (h && h.then ? h.then((a) => this._PostResultToRuntime(e, !0, a)).catch((a) => {
                console.error(`Rejection from '${b}' handler '${c}':`, a), this._PostResultToRuntime(e, !1, a.toString())
...
"use strict"; {
    ...
    const r = class extends DOMHandler {
        ...
        _OnPointerEvent(a, b) {
            "pointerdown" === a && window !== window.top && window.focus(), this._pointerRawUpdateRateLimiter && ("pointerup" === a || "pointercancel" === a) && this._pointerRawUpdateRateLimiter.Reset();
            let c = 0;
...
"use strict"; {
    ...
    const b = class extends DOMHandler {
        constructor(b) {
            super(b, "share");
            const c = !!this._GetSharePlugin();
            this._isWebShareSupported = "function" == typeof navigator["share"], this._isSupported = this._isWebShareSupported || c, this._isWebShareFilesSupported = a(), this._isFilesSupported = this._isWebShareFilesSupported || c, this.AddRuntimeMessageHandlers([
                ["init", (a) => this._OnInit(a)],
                ["share", (a) => this._OnShare(a)],
                ["request-rate", (a) => this._OnRateApp(a)],
                ["request-store", (a) => this._OnShowStore(a)]
            ])
        }
        _OnInit() {
            return {
        ...
        async _OnShare(a) {
            const b = a["text"],
                ...
                e = a["files"].slice(0),
                f = 0 < e.length,
                g = this._GetSharePlugin();
            if (!g) {
                const a = {};
                b && (a["text"] = b), c && (a["title"] = c), d && (a["url"] = d), f && (a["files"] = e);
                try {
                    await navigator["share"](a), this.PostToRuntime("share-completed")
                } catch (a) {
                    console.log("[Share plugin] Share failed: ", a), this.PostToRuntime("share-failed")
                }
            } else {
                const a = {};
                if (b && (a["message"] = b), c && (a["subject"] = c), d && (a["url"] = d), f) {
                    let b = 0;
                    for (const a of e) b += a.size;
                    try {
                        const c = await this._CordovaGetTempDirEntry(Math.floor(1.25 * b));
                        a["files"] = await Promise.all(e.map((a) => this._CordovaWriteTempFile(a, c)))
                    } catch (a) {
                        return console.log("[Share plugin] Share failed: ", a), void this.PostToRuntime("share-failed")
                    }
                }
                g["shareWithOptions"](a, () => {
        ...
        _OnRateApp(a) {
            const b = a["body"],
                ...
                d = a["cancel"],
                e = a["appID"],
                f = this._GetRatePlugin();
            f && f["Rate"](b, c, d, e)
        }
        _OnShowStore(a) {
            const b = a["appID"],
                c = this._GetRatePlugin();
            c && c["Store"](b)
        }
        _CordovaGetTempDirEntry(a) {
            return new Promise((b, c) => {
                window["requestFileSystem"](window["TEMPORARY"], a, (a) => b(a["root"]), c)
            })
        }
        _CordovaWriteTempFile(a, b) {
            return new Promise((c, d) => {
                b["getFile"](a.name, {
                    ...
                    const e = b["toURL"]();
                    b["createWriter"]((b) => {
                        b["onwriteend"] = () => c(e), b["onerror"] = d, b["write"](a)
                    })
                }, d)
            })
        }
...
"use strict"; {
    ...
    self.C3AudioInstance = class {
        constructor(a, b, c) {
            ...
            const d = this._audioDomHandler.GetTimeScaleMode();
            this._isTimescaled = 1 === d && !this.IsMusic() || 2 === d, this._instUid = -1, this._fadeEndTime = -1, this._stopOnFadeEnd = !1
        }
        Release() {
        ...
        Tick(a) {
            -1 !== this._fadeEndTime && a >= this._fadeEndTime && (this._fadeEndTime = -1, this._stopOnFadeEnd && this.Stop(), this._audioDomHandler.PostTrigger("fade-ended", this._tag))
        }
        GetOverallVolume() {
...
self.C3WebAudioInstance = class extends C3AudioInstance {
    ...
    _OnEnded(a) {
        this._isPaused || this._resumeMe || a.target !== this._activeSource || (this._hasPlaybackEnded = !0, this._isStopped = !0, this._instUid = -1, this._ReleaseBufferSource(), this._audioDomHandler.PostTrigger("ended", this._tag))
    }
    HasEnded() {
